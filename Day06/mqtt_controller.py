...
class publisher(Thread):
    def __init__(self):
        ...
        self.host = '210.119.12.59'   # 본인 PC ip주소 적기
        self.port = 1883  # 회사에서는 mqtt포트 그대로 쓰면 안됩니다
        self.clientId = 'IOT59'
        self.count = 0
        print('publisher 스레드 시작')
        self.client = mqtt.Client(client_id= self.clientId)   # client id는 설계대로

    def run(self):
        self.client.connect(self.host, self.port)
        # self.client.username_pw_set()     # id/pwd로 로그인 할때는 필요
        self.publish_data_auto()

    def publish_data_auto(self):
        humid, temp = dht.read_retry(sensor, rcv_pin)
        curr = dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 2023-06-14 10:39:40 이런느낌
...
class subscriber(Thread):
    def __init__(self):
        ...
        self.host = '210.119.12.59'     # Broker IP

        self.port = 1883
        self.clientId = 'IOT59_SUB'
        self.topic = 'pknu/monitor/control/'
        print('subscriber 스레드 시작')
        self.client = mqtt.Client(client_id= self.clientId)

    def run(self):  # Thread.start() 함수를 실행하면 실행되는 함수
        self.client.on_connect = self.onConnect  # 접속 성공시그널 처리
